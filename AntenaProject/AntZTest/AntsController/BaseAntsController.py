...
def handle_exception(exc_type, exc_value, exc_traceback):
    ...
    return ComposedEvaluationResponse.GetExceptionResult(
        format(f"exc_type: {exc_type} exc_value:{exc_value} exc_traceback:{exc_traceback}"))
...
class BaseAntsController(ABC):
    ...
    def Process(self) -> ComposedEvaluationResponse:
        with PerformanceCounter("Process", self._performance_writter_wrapper):
            ...
            self._ants_meta_data_consumer.ProcessPreRun(self._num_of_steps, self._Maze,
                                                        self._GetPreTestAdditionalData())
            counter = 0
            ...
            while counter < self._num_of_steps:
                with PerformanceCounter("Colony_Step", self._performance_writter_wrapper):
                    ...
                    self._ants_meta_data_consumer.ProcessPostSysStep(counter,
                                                                     self._world_image_provider.GetWorldImage(),
                                                                     self._GetPostStepAdditionalData())

            self._ants_meta_data_consumer.ProcessPreStopRun(self._num_of_steps,
                                                            self._world_image_provider.GetWorldImage(),
