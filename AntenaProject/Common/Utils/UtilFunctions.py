    ...
    if dx >= dy:
        while x0 != x1:
            ...
            if f != 0 and isBlocked(combined_map[int(x0+(sx-1)/2), int(y0+(sy-1)/2)]):
                return False
            if dy == 0 and isBlocked(combined_map[int(x0+(sx-1)/2), int(y0)]) and isBlocked(combined_map[int(x0+(sx-1)/2), int(y0-1)]):
                return False
            x0 = x0 + sx
    else:
        while y0 != y1:
            ...
            if f != 0 and isBlocked(combined_map[int(x0+(sx-1)/2), int(y0+(sy-1)/2)]):
                return False
            if dy == 0 and isBlocked(combined_map[int(x0), int(y0+(sy-1)/2)]) and isBlocked(combined_map[int(x0-1), int(y0+(sy-1)/2)]):
                return False
            y0 = y0 + sy
    return True
